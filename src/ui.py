...
架構原則：
  - UI 只呼叫兩個後端介面（與 engine.py 的真實簽名完全對齊）：
      1. search_callback(user_input: str) -> tuple[str, list[float]]  # RAGEngine.search
         （回傳合併判決文字，以及對應的 FAISS L2 距離列表）
...
# ✅ 修改後：同時 import 這兩個函式
from src.engine import get_legal_summary, check_input_sanity
...
def _estimate_case_similarity(distances: list) -> float:
    """
    從 RAGEngine.search 回傳的 FAISS L2 距離估算「案例相似度」。
    L2 距離越小代表語意越接近，以 1 / (1 + distance) 映射到 (0, 1] 區間，
    多筆取平均代表整體檢索品質。
    ※ 此為真實檢索距離換算，不再以「找到幾筆」作為代理指標。
    """
    if not distances:
        return 0.0
...
def _estimate_info_completeness(user_input: str) -> float:
    ...
    if len(user_input) > 30:
        score += 0.15
    # 以 set 去重，避免同一關鍵詞重複出現即可無限刷分
    matched_kws = {kw for kw in ["折舊", "賠償", "違約", "逸失", "條款"] if kw in user_input}
    score += min(len(matched_kws) * 0.05, 0.10)
    return min(round(score, 2), 1.0)


def _estimate_confidence_score(case_similarity: float, info_completeness: float) -> float:
    """
    以調和平均數（Harmonic Mean）整合『判決匹配度』與『資訊完整度』。

    參考 F1-Score 等評估框架中「調和平均可懲罰極端值」的設計精神：
    任一維度趨近 0 時，整體分數會被拉低，避免單一高分掩蓋另一維度的不足
    （例如：找到很多筆判決但使用者輸入極度空泛，或輸入很完整但完全找不到
    相似判決，這兩種情況都不該得到高信心分數）。

    註：此為自訂啟發式指標，非直接套用 RAGAS 等業界框架的 LLM 評分機制。
    """
    if case_similarity <= 0 or info_completeness <= 0:
        return 0.0
    return round(
        2 * (case_similarity * info_completeness) / (case_similarity + info_completeness),
        3,
    )


def _estimate_confidence(combined_score: float) -> int:
    """依調和平均分數換算 AI 信心指數（1～5 顆星）。"""
...
def _build_analysis_results(
    ...
    輸入：engine.py 兩個函式的真實回傳值（含 RAGEngine.search 的 FAISS 距離）。
    輸出：所有 UI 區塊共用的資料來源。

    ── 後端整合說明 ──
    若未來 engine.py 新增其他結構化資料，只需在此函式替換對應的推導邏輯，
    UI 渲染層完全不用動。
    """
    case_similarity   = _estimate_case_similarity(distances)
    info_completeness = _estimate_info_completeness(user_input)
    final_risk_score  = _estimate_confidence_score(case_similarity, info_completeness)
    ai_confidence     = _estimate_confidence(final_risk_score)
...
def _render_quantitative_dashboard(results: dict):
    ...
    with c2:
        ...
        st.metric(
            ...
            delta_color="normal",
            help="由 RAGEngine.search 回傳的 FAISS L2 距離換算而得。"
                 "距離越小代表語意越相近，多筆結果取平均。",
        )
    ...
    with c4:
        # ⚠️ st.metric 的 value 欄位在欄寬不足時會被截斷成刪節號，
        # 不管字串多短都可能發生。改成跟 c1 風險徽章一樣的做法：
        # st.metric 只放精簡數字，星星圖案改用不受欄寬限制的 st.markdown 呈現。
        st.metric(
            label="🤖 AI 信心指數",
            value=f"{conf} / 5",
            delta="Seed 42 / 穩定",
            delta_color="off",
            help="以調和平均分數（判決匹配度、資訊完整度）換算，temperature=0, seed=42。",
        )
        stars = "⭐" * conf + "☆" * (5 - conf)
        st.markdown(
            f"""<div style="
    ...
    with st.container(border=True):
        if sim > 0 and comp > 0:
            formula_block = f"""```
AI信心指數 = 2 × (判決匹配度 × 資訊完整度) / (判決匹配度 + 資訊完整度)
           = 2 × ({sim:.2f} × {comp:.2f}) / ({sim:.2f} + {comp:.2f})
           = {final:.3f}
```"""
        else:
            formula_block = f"""```
AI信心指數 = 2 × (判決匹配度 × 資訊完整度) / (判決匹配度 + 資訊完整度)
           = 0（任一指標為 0，依調和平均定義直接歸零）
```"""
        st.markdown(
            f"""**AI信心指數計算公式（調和平均數 Harmonic Mean）：**
{formula_block}
> 採用調和平均數而非算術平均，任一指標偏低時會被拉低整體分數，避免單一高分掩蓋另一維度的不足
> （設計精神參考 F1-Score 等評估框架；本指標仍為自訂啟發式計算，非直接套用 RAGAS 等業界 LLM 評分框架）。"""
        )
...
def run_ui(search_callback):
    ...
    search_callback：RAGEngine.search，簽名為 (query: str) -> (str, list[float])
    """
    st.set_page_config(
            ...
            if not user_input.strip():
                ...
                if not sanity_check["is_valid"]:
                    ...
                    try:
                        ...
                        # 1. RAGEngine.search(query) -> (str, list[float])
                        #    現在同時回傳合併文字與真實 FAISS L2 距離
                        raw_documents, distances = search_callback(user_input)
                        # 2. get_legal_summary(query, docs) -> str
                        ai_report = get_legal_summary(user_input, raw_documents)

                        # ── 組裝 analysis_results（所有 UI 的唯一資料來源）──
                        results = _build_analysis_results(user_input, raw_documents, distances, ai_report)
